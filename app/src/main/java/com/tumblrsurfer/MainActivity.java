...
private boolean CheckInternetConnection() 
{
      ...
      connec.getNetworkInfo(1).getState() == android.net.NetworkInfo.State.CONNECTED ) {
         Toast.makeText(ItsContext, " Connected ", Toast.LENGTH_LONG).show();
         return true;
      }else if (
      connec.getNetworkInfo(0).getState() == android.net.NetworkInfo.State.DISCONNECTED ||
      connec.getNetworkInfo(1).getState() == android.net.NetworkInfo.State.DISCONNECTED  ) {
         Toast.makeText(ItsContext, " Not Connected ", Toast.LENGTH_LONG).show();
         return false;
      }
      return false;
}	
//--------------------------------------------------------------------------------------------------
public boolean SaveFileFolder( String filename, String outputfolder, boolean externalstorageflag, byte[] bytedata )
{
   boolean successflag = true;
   String path = "";
   
   if( externalstorageflag )  
	  path = Environment.getExternalStorageDirectory() + "/" + outputfolder;   
   else
	 path = ItsContext.getFilesDir() + "/" + outputfolder;
   ...
   if (!finalpath.exists()) 
      successflag = finalpath.mkdir();

  if (successflag) 
  {
	    finalpath.setReadable( true, false );
	  
        try
        {        	
           File finalpathfile = new File(finalpath + "/" + filename);           
           finalpathfile.createNewFile();
           FileOutputStream fileout = new FileOutputStream(finalpathfile);
...
//--------------------------------------------------------------------------------------------------
private ByteArrayOutputStream OpenHttp(String urlstr, int connecttype, String arguments, String[] headerout  ) throws Exception
{
...
if( connecttype == HTTP_GET )
{
        ...
        int bytesread = -1;
		InputStream inputstreamobj = con.getInputStream();
		ByteArrayOutputStream bytearrayoutputstreamobj = new ByteArrayOutputStream();
		byte[] bytearray = new byte[1024];
		int totalbytesread = 0;
		
		while( (bytesread = inputstreamobj.read( bytearray )) > -1 )
		{
		  bytearrayoutputstreamobj.write( bytearray, 0, bytesread );
		} 
       ...
       if (responseCode == HttpURLConnection.HTTP_OK)
		...
		return bytearrayoutputstreamobj;
}
else
...
protected void GotoUrl(final String urlstr, final int connecttype, final String arguments, final short arrayindex ) 
{
      ...
      final ByteArrayOutputStream[] bytearrayoutputstreamobj = new ByteArrayOutputStream[1];
      ...
      new Thread() {
         public void run() {
            ...
            try {
               bytearrayoutputstreamobj[0] = OpenHttp(url, connecttype, arguments, headerout );
               
               if( bytearrayoutputstreamobj[0] == null )                          	   
               {            	      	   
               ...
               ByteArrayList[arrayindex] = bytearrayoutputstreamobj[0].toByteArray();
               HeaderList[arrayindex][0] = new String(headerout[0]);
               Bundle b = new Bundle();
...
//--------------------------------------------------------------------------------------------------  
protected void DownloadFile(final String urlstr, final int connecttype, final String arguments,
		   final String outputfolder, final boolean externalstorageflag ) 
{	  
      final String url = urlstr;
      final String[] headerout = { "" };
      final String[] outputstr = { "" };
      final byte[][] outputbytes = {{0}};
      ...
      new Thread() {
         public void run() {
            ...
            try {
               outputbytes[0] = OpenHttp(url, connecttype, arguments, headerout ).toByteArray();
               
               if( outputbytes[0] == null )                          	            	      	             
            	   return;                       
            }
            ...
            SaveFileFolder( GetUrlFileName(urlstr), outputfolder, externalstorageflag, outputbytes[0] );
         }
      }.start();
   }
//--------------------------------------------------------------------------------------------------  
protected String GetUrlFileName( String url )
{
   ...
   for( i = urlbytes.length - 1; i >= 0; i-- )
	 if( urlbytes[i] == '/' )
	 {
		retstr = url.substring(i + 1); 
		break; 
	 }
...
protected boolean UseBigImagesFlag = false;
...
public void Search( String keywordstr, int searchtype )
{
     ...
     if( searchtype == RESET_SEARCH || StopFlag == 1 )
     {
    	StopFlag = 0;
        ClearStacks();       
        return;
     ...
     {     	
       ...
       {    
    	  ...
    	  if( buffer[0].indexOf( "\"status\":404" ) < 0 || buffer[0].length() >= 30 )
    	  {
    	   do
    	   {
    		 subbuffer[0] = ExtractBraceGroupJson( buffer, "{\"blog_name\":\"", '{', '}' );
...
String[] CreatePost( String[] blogdata )
{
  ...
  String blogimagelink = "", imagelinkstr = "";
  String bigimagelist = "";
...
for( j = 0; j < 20; j++ )
{
  blogcaption = StrCut( "\"caption\":\"", "\"", blogdata );
  ...
  if( UseBigImagesFlag )
	  imageurl = blogimagelist[2];
  else
	  imageurl = blogimagelist[4];
  
  imagelinkstr +=  "<a href=\"" + imageurl + "\" >" + "<img src=\"" + imageurl + "\" />" + "</a>";
  bigimagelist += "[bigimage]" + blogimagelist[0] + "[/bigimage]";
}

  String[] outputhtml = { "<center><b>" + blogname + "</b> " + blogdate + "<br><i>" + blogtags+ "</i><br>" 
         + blogcaption + "<br>" + imagelinkstr + "</center>" + "<!--" + bigimagelist + "-->" };
...
//--------------------------------------------------------------------------------------------------
protected String CreateTumblrApiUrl( String blogid, String postid, String posttype )
{
...
for(x = 0; x < layoutchildcount - 1; x++) 
{
   viewobj = ll.getChildAt(x);   
   customwebview = (CustomWebView)viewobj.findViewById(R.id.WebView1);
    	  
   for( i = 0; i < 100; i++ )
   {
      url = StrCut( "[bigimage]", "[/bigimage]", customwebview.Data );
      if( url == null )
    	  break;
    		
       TumblrObj.InternetObj.DownloadFile(url, Internet.HTTP_GET, "", "Tumblr", true );     		 
   }   
}
...
LinearLayout1.addView(tumblrview);
    ...
    saveimagebutton.setOnClickListener(new OnClickListener(){
     ...
     public void onClick(View v) {
      ...
      if( customwebview.Data[0].indexOf( "<!--error page-->") < 0 )
      {
    	  int i; 
    	  String url;
...
checkbox1.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() 
{
       ...
       {
    	 if( isChecked )
    		 TumblrObj.UseBigImagesFlag = !TumblrObj.UseBigImagesFlag;
       }
   }
);     
}
